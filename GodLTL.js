...
/** Test for Node.replicate() **/
function testReplicate() {
    let testNode = new Node(2, 'and', null, null, null);
    let testLNode = new Node(0, 'A', null, null, null);
    let testRNode = new Node(0, 'B', null, null, null);
    testNode.addLc(testLNode);
    testNode.addRc(testRNode);

    let repNode = testNode.replicate();
    testLNode.val = 'D';
    console.log('Tree origin: ' + testNode.val + ' ' + testNode.lc.val + ' ' + testNode.rc.val);
    console.log('Tree replicate: ' + repNode.val + ' ' + repNode.lc.val + ' ' + repNode.rc.val);
...
function judge(s0, task) {
    ...
    // hacky way to implement retrying in 'eventually' and 'always'
    // if (RETRY_SWITCH === true)
    //     res.s1 = s0;
        ...
        for (let k=0; k<roots.length; k++) {
            ...
            for (let round = 0; round < 25; round++) {
                ...
                for (let i=tmpLen-1; i>=0; i--) {
                    if (feedbackTable[idx * tmpLen + i].s1 !== trainerFeedback.s1 ||
                        feedbackTable[idx * tmpLen + i].term !== trainerFeedback.term) {
...
let targetTask = task2();

// mat = generateMatrix(5);
// printMatrix(mat, 5);
// test();
// main(targetTask);
// console.log(judgeTerm(11,10,nodeTest))
// console.log(targetTask.getQuadTuple());
// printDistCoef(mat, 'A');
...
let testTaskList = makeTasksWithDepth(4);
