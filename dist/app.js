...
var _GridPainter = require('./js/GridPainter');
...
(0, _jquery2.default)(document).ready(function () {
    ...
    var agent = new _LTL_interactive.ILTL({
        targetTask: _LTL_interactive.ILTL.task3()
    });
    var painter = new _GridPainter.GridPainter((0, _jquery2.default)('#grid-container'), {
        clickCallBack: function clickCallBack(index) {
    ...
    var $btnPositive = (0, _jquery2.default)('#btn-positive'),
        $btnNegative = (0, _jquery2.default)('#btn-negative'),
        $btnReplace = (0, _jquery2.default)('#btn-replace'),
        $btnReset = (0, _jquery2.default)('#btn-reset'),
        $btnRevert = (0, _jquery2.default)('#btn-revert');

    $btnPositive.click(function () {
        var res = agent.filterByFeedback(true, s1);
        if (res === 0) {
            s0 = s1;
    ...
    $btnNegative.click(function () {
        var res = agent.filterByFeedback(false, s1);
        if (res === 0) {
            s0 = s1;
            s1 = agent.calcNextMove(s0.s1);
            if (s1.s1 !== -1) {
    ...
    function displayPrediction(state) {
        // positive
        var res = agent.predictByFeedback(true, state);
...
function log(str) {
    ...
    var $p = (0, _jquery2.default)('<p>' + str + '</p>'),
        $logger = (0, _jquery2.default)('#logger'),
        $logs = (0, _jquery2.default)('#logger-logs');
    $p.css('color', color);
    $logs.append($p);
    var deltaH = $logs.height() - $logger.height();
    $logger.scrollTop(deltaH > -40 ? deltaH + 40 : 0);
}

},{"./js/GridPainter":3,"./js/LTL_interactive":4,"jquery":1}],3:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
    value: true
});
exports.GridPainter = undefined;
...
var GridPainter = exports.GridPainter = function () {
    function GridPainter($container, option) {
        _classCallCheck(this, GridPainter);

        if (!$container) console.error('GridPainter.constructor(): $container cannot be empty.');
        ...
        this.androidSize = 75;
        this.android = (0, _jquery2.default)('<div></div>').addClass('grid-android').css({
    ...
    _createClass(GridPainter, [{
        ...
        value: function paintGrid() {
            ...
            for (var h = 0; h < this.height; h++) {
                var $row = (0, _jquery2.default)('<div></div>');
                $row.addClass('grid-row').attr('data-row', h);
                this.container.append($row);

                var _loop = function _loop(w) {
                    var $grid = (0, _jquery2.default)('<div></div>'),
                        gridIndex = h * _this.width + w;
                    $grid.addClass('grid').attr('data-grid', gridIndex).click(function () {
                        _this.gridClicked(gridIndex);
                    }).css({
                        height: _this.gridSize + 'px',
                        width: _this.gridSize + 'px',
                        'line-height': _this.gridSize + 'px'
                    });
        ...
        key: '_paintChar',
        value: function _paintChar(index) {
            var char = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '?';

            var $grid = this.gridList[index];
            $grid.append('<span>' + char + '</span>');
        }
    }, {
        key: 'paintMatrix',
        value: function paintMatrix(mat) {
            this.container.find('span').remove();
            for (var i = 0; i < this.height; i++) {
                for (var j = 0; j < this.width; j++) {
                    var index = i * this.width + j;
                    if (mat[index] === 'A' || mat[index] === 'B' || mat[index] === 'C' || mat[index] === 'D') this._paintChar(index, mat[index]);
                }
            }
        }
    }, {
        key: 'gridClicked',
        value: function gridClicked(index) {
            if (this.freeze) return;
            if (!!this.clickCallBack) this.clickCallBack(index);
        }
    }, {
        key: '_getGridPos',
        value: function _getGridPos(index) {
        ...
        key: 'setAndroid',
        value: function setAndroid(index) {
            var pos = this._getGridPos(index);
            this.android.css({
        ...
        key: 'clearAndroid',
        value: function clearAndroid() {
            this.container.find('.grid-android').remove();
        }
    }, {
        key: 'moveAndroid',
        value: function moveAndroid(destIdx) {
            if (destIdx < 0) {
                if (destIdx === -2) this.setAndroidEmotion(0);else if (destIdx === -3) this.setAndroidEmotion(1);
                return;
            }
            var destPos = this._getGridPos(destIdx);
        ...
        key: 'freezePainter',
        value: function freezePainter() {
            this.freeze = true;
        }
    }, {
        key: 'unfreezePainter',
        value: function unfreezePainter() {
            this.freeze = false;
        }
    }, {
        key: 'setAndroidEmotion',
        value: function setAndroidEmotion() {
            var emotion = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;

            var clr = 'green';
            if (emotion === 0) clr = 'green';else if (emotion === 1) clr = 'red';else clr = 'orange';
...
var ILTL = exports.ILTL = function () {
    ...
    _createClass(ILTL, [{
        ...
        value: function init() {
            ...
            this.subTask.sort(function (node1, node2) {
                return node1.depth < node2.depth ? 1 : -1;
            });
        }
    }, {
        key: 'initSubTask',
        value: function initSubTask() {
            var self = this;
            ...
            function cleanSubTask(taskArr, task) {
                for (var g = 0; g < taskArr.length; g++) {
                    // task = self.subTask[g];
                    if (!!task) {
                        for (var _i = taskArr.length - 1; _i >= 0; _i--) {
                            if (taskArr[_i].op === 0) ;else if (task.op === 1) {
                                if (task.lc.equal(taskArr[_i])) taskArr.splice(_i, 1);
                            } else if (task.op === 2) {
                                if (task.lc.equal(taskArr[_i]) || task.rc.equal(taskArr[_i])) taskArr.splice(_i, 1);
                            }
                        }
                    }
                }
        ...
        key: 'calcNextMove',
        value: function calcNextMove(pos) {
            if (pos === -1) {
            ...
            var tmpLen = this.TBDTask.length;
            ...
            var sumMap = new Map(),
                maxSum = -1,
                nextState = null;
            for (var _i2 = 0; _i2 < tmpLen; _i2++) {
                var nowFeedback = this.feedbackTable[_i2];
                var res = sumMap.get(nowFeedback);

                // already used this grid
                if (this.validGrid.indexOf(nowFeedback.s1) === -1) continue;

                if (res === undefined) sumMap.set(nowFeedback, 1);else sumMap.set(nowFeedback, res + 1);
            ...
            if (nextState !== null) return nextState;else {
                console.log('DEAD!!');
                alert('Failed!');
                return {
                    s1: -1,
        ...
        key: 'filterByFeedback',
        value: function filterByFeedback(fb, state) {
            console.log(state);
            var tmpLen = this.TBDTask.length;
            for (var i = tmpLen - 1; i >= 0; i--) {
                var thisState = this.feedbackTable[i];
                if (fb === true) {
                    if (state.term === 0) {
                        if (thisState.s1 !== state.s1 || thisState.term !== state.term) this.TBDTask.splice(i, 1);
                    } else {
                        if (thisState.term !== state.term) this.TBDTask.splice(i, 1);
                    }
                } else {
                    if (state.term === 0) {
                        if (thisState.s1 === state.s1 && thisState.term === state.term) this.TBDTask.splice(i, 1);
            ...
            if (this.TBDTask.length === 1) {
                ...
                if (this.currentTask.equal(this.targetTask)) {
                    if (this.finish instanceof Function) console.log("Finished");
                    this.finish();
                }
        ...
        key: 'predictByFeedback',
        value: function predictByFeedback(fb, state) {
            var res = [];
            var tmpLen = this.TBDTask.length;
            for (var i = tmpLen - 1; i >= 0; i--) {
                var thisState = this.feedbackTable[i];
                if (fb === false) {
                    if (state.term === 0) {
                        if (thisState.s1 !== state.s1 || thisState.term !== state.term) res.push(this.TBDTask[i].print(false));
                    } else {
                        if (thisState.term !== state.term) res.push(this.TBDTask[i].print(false));
                    }
                } else {
                    if (state.term === 0) {
                        if (thisState.s1 === state.s1 && thisState.term === state.term) res.push(this.TBDTask[i].print(false));
        ...
        value: function generateMatrix() {
            ...
            // generate C position
            var randC = parseInt(4 * Math.random());
            // let randC = 1;
            ...
            for (var _i5 = 1; _i5 < n - 1; _i5++) {
                idxD.push(n * _i5);
                idxD.push(n * _i5 + n - 1);
            }
            ILTL.removeByValue(idxD, posDicC[randC]);
            var randD = parseInt(idxD.length * Math.random());
            // let randD = 5;
            ...
            //generate A B
            var randA = parseInt((n * n - 2) * Math.random());
            // let randA = 10;
            ...
            var randB = parseInt((n * n - 3) * Math.random());
            // let randB = 15;
        ...
        key: '_makeNewTasks',
        value: function _makeNewTasks() {
            var resTaskList = [];

            // assemble original tasks with 'and', 'eventually', 'or', 'always', 'not'
            for (var i = 0; i < this.defaultTask.length; i++) {
                var L = this.defaultTask[i];
                if (L.val !== 'always' && L.val !== 'not') resTaskList.push(ILTL.hypoAlways(L));
                if (L.val !== 'eventually' && L.val !== 'not') resTaskList.push(ILTL.hypoEventually(L));
                if (L.val !== 'not') resTaskList.push(ILTL.hypoNot(L));

                for (var j = i + 1; j < this.defaultTask.length; j++) {
                    // need to delete LTLs like 'T and always T', 'T or always T'
                    var R = this.defaultTask[j];
                    if (R.val === 'always') {
                        if (L.equal(R.lc)) {
                            continue;
                        }
                    ...
                    if ((L.atoms & R.atoms) === 0) {
                        ...
                        if (flag === true) resTaskList.push(ILTL.hypoAnd(L, R));

                        flag = true;
                        if (L.val === R.val) flag = false;
                        if (flag === true) resTaskList.push(ILTL.hypoOr(L, R));
                    }
                }
            }
        ...
        key: '_distScore',
        value: function _distScore(s0, distChar, qTuple) {
            var d = this._dist(s0, distChar);
        ...
        key: '_dist',
        value: function _dist(src, distChar) {
            var target = -1;
        ...
        key: '_judge',
        value: function _judge(s0, task) {
            // console.log('[_judge]: s0 = ' + s0, task);
            var res = { s1: null, term: null };
            var self = this;
            ...
            function judgeState(s0, task) {
                var quadTuple = task.getQuadTuple();
                var dests = [];

                // validate 5 potential destinations, left -> up -> right -> down
                if (s0 % 5 !== 0) dests.push(s0 - 1);
                if (s0 - 5 >= 0) dests.push(s0 - 5);
                if ((s0 + 1) % 5 !== 0) dests.push(s0 + 1);
                if (s0 + 5 < 25) dests.push(s0 + 5);

                var tmpCnt = 0;
                var occupy = false;
                for (var key in quadTuple) {
                ...
                try {
                    for (var _iterator = dests[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                        var dest = _step.value;

                        var score = 0;
                        for (var _key3 in quadTuple) {
                            if (quadTuple[_key3] !== null) {
                                score += self._distScore(dest, _key3, quadTuple);
            ...
            function judgeTerm(s0, task) {
                var res = void 0;
                if (task.op === 0) {
                    if (self.mat[s0] === task.val) return 1;else return -1;
                }
                ...
                if (task.op === 1) {
                    switch (task.val) {
                        case 'not':
                            ...
                            if (res === 1) return -1;else if (res === 0) return 0;else return 1;
                            break;
                        case 'eventually':
                            res = judgeTerm(s0, task.lc);
                            // console.log(res);
                            if (res === 1) return 1;else if (res === -1) {
                                var rand = Math.random();
                                // console.log(rand,' ', self.miu);
                                if (rand < self.miu) return -1;else {
                                    return 0;
                                }
                            } else if (res === 0) {
                                var _rand = Math.random();
                                if (_rand < self.miu) return -1;else return 0;
                            }
                            break;
                        case 'always':
                            res = judgeTerm(s0, task.lc);
                            if (res === -1) return -1;else if (res === 1) {
                                ...
                                if (_rand2 < 1 - self.theta) {
                                    return 0;
                                } else return 1;
                            } else if (res === 0) {
                                var _rand3 = Math.random();
                                if (_rand3 < self.theta) return 1;else return 0;
                            }
                ...
                if (task.op === 2) {
                    ...
                    switch (task.val) {
                        case 'and':
                            ...
                            if (res1 === 1) if (res2 === 1) return 1;
                            if (res1 === -1 || res2 === -1) return -1;
                            return 0;
                            break;
                        case 'or':
                            var nodeNG = ILTL.hypoNot(task.lc);
                            var nodeNT = ILTL.hypoNot(task.rc);
                            var nodeAnd = ILTL.hypoAnd(nodeNG, nodeNT);
                            var nodeTarget = ILTL.hypoNot(nodeAnd);
        ...
        key: '_generateSubtrees',
        value: function _generateSubtrees(node) {
            if (!!node.lc) this._generateSubtrees(node.lc);
            if (!!node.rc) this._generateSubtrees(node.rc);

            if (node.op > 0) {
                var tmpNode = node.replicate();
                tmpNode.pa = null;
        ...
        key: 'hypoAlways',
        value: function hypoAlways(node) {
            var always = new Node(1, 'always', null, null, null);
            always.addLc(node.replicate());
            return always;
        }
    }, {
        key: 'hypoEventually',
        value: function hypoEventually(node) {
            var eventually = new Node(1, 'eventually', null, null, null);
            eventually.addLc(node.replicate());
            return eventually;
        }
    }, {
        key: 'hypoNot',
        value: function hypoNot(node) {
            var not = new Node(1, 'not', null, null, null);
            not.addLc(node.replicate());
            return not;
        }
    }, {
        key: 'hypoAnd',
        value: function hypoAnd(node1, node2) {
            var and = new Node(2, 'and', null, null, null);
        ...
        key: 'hypoOr',
        value: function hypoOr(node1, node2) {
            var or = new Node(2, 'or', null, null, null);
        ...
        value: function task1() {
            var targetTask = new Node(2, 'and');
            var nodeC = new Node(0, 'C');
            var nodeA = new Node(0, 'A');
            var nodeB = new Node(0, 'B');

            var nodeEvC = ILTL.hypoEventually(nodeC);
            var nodeAB = ILTL.hypoOr(nodeA, nodeB);
            var nodeNot = ILTL.hypoEventually(nodeAB);
            var nodeAlw = ILTL.hypoNot(nodeNot);
        ...
        value: function task2() {
            ...
            var nodeEv = new Node(1, 'eventually');
            var nodeA = new Node(0, 'A');

            var nodeEv2 = new Node(1, 'eventually');
            var nodeAlw = new Node(1, 'always');
            var nodeD = new Node(0, 'D');
        ...
        key: 'task3',
        value: function task3() {
            var targetTask = new Node(1, 'always');
            var nodeC = new Node(0, 'C');
            var nodeD = new Node(0, 'D');

            var nodeEv = ILTL.hypoEventually(nodeD);
            var nodeAnd = ILTL.hypoAnd(nodeC, nodeEv);

            var nodeEv2 = ILTL.hypoEventually(nodeAnd);
        ...
        key: 'task4',
        value: function task4() {
            var targetTask = new Node(2, 'and');
            var nodeD = new Node(0, 'D');
            var nodeA = new Node(0, 'A');
            var nodeB = new Node(0, 'B');

            var nodeRight = ILTL.hypoNot(ILTL.hypoEventually(nodeD));
            var nodeLeft = ILTL.hypoEventually(ILTL.hypoAnd(nodeA, ILTL.hypoEventually(nodeB)));
        ...
        key: 'task5',
        value: function task5() {
            var nodeA = new Node(0, 'A');
            var nodeB = new Node(0, 'B');

            var nodeLeft = ILTL.hypoAlways(ILTL.hypoEventually(ILTL.hypoAnd(nodeA, ILTL.hypoEventually(nodeB))));
